...
const startSound = new Audio('./audio/boxing_ring.wav');
const hitSounds = [
...
const player = new Fighter({
    ...
    sprites: {
        idle: {
            imageSrc: './img/samuraiMack/Idle.png',
            framesMax: 4,
            scale: 0.4, // Adjust this value to fit the sprite within the canvas
...
function enemyAI() {
    ...
    if (enemyDistanceFromCenter < -50) {
        enemy.velocity.x = 3;
        enemy.switchSprite('run');
    } else if (enemyDistanceFromCenter > 50) {
        enemy.velocity.x = -3;
        enemy.switchSprite('run');
    } else {
        enemy.velocity.x = 0;
        ...
        if (!enemy.isAttacking && Math.random() < 0.2) {
            const randomAction = Math.random();
            if (randomAction < 0.6) {
                enemy.attack();
            } else if (randomAction < 0.8 && enemy.position.y === 330) {
                enemy.velocity.y = -15;
            }
        }
    }
